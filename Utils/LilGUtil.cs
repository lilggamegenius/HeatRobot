...
namespace HeatBot.Utils{
	...
	public static class LilGUtil{
		...
		public static bool CheckIfOwner(PrivateMessage privateMessage){
			return privateMessage.User.Nick.EqualsIgnoreCase(Program.Config.botOwnerName);
		}

		public static bool CheckPermission(string commandName, PrivateMessage e){
			if(CheckIfOwner(e)){
			...
			if(permission.Level == Level.Broadcaster){ return false; }
			if(permission.Level == Level.Viewer){ return true; }
			...
			//return
			return false; //Todo check if the user is mod
		}
